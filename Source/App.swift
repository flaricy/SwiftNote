...
final class AppController: NSObject, NSApplicationDelegate, NSWindowDelegate, NSTableViewDataSource, NSTableViewDelegate, NSSearchFieldDelegate {
    ...
    var localizedButtons: [(NSButton, String)] = []
    var localizedLabels: [(NSTextField, String)] = []

    let table = NSTableView()
    let search = NSSearchField()
    let status = NSTextField(labelWithString: "")
    let imageSlider = NSSlider(value: 400, minValue: 80, maxValue: 900, target: nil, action: nil)
    let imageLabel = NSTextField(labelWithString: L("图片宽度"))
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if ProcessInfo.processInfo.environment["SWIFTNOTE_FIXTURE"] != nil { window.orderFront(nil) }
    ...
    func buildMenu() {
        ...
        add(app, L("关于随记"), #selector(about), "")
        add(app, L("设置…"), #selector(showSettings), ",")
        app.addItem(.separator()); add(app, L("隐藏随记"), #selector(NSApplication.hide(_:)), "h", target: NSApp)
        app.addItem(.separator()); add(app, L("退出随记"), #selector(quit), "q")
        let file = NSMenu(title: L("文件")); let fileItem = NSMenuItem(title: L("文件"), action: nil, keyEquivalent: ""); fileItem.submenu = file; main.addItem(fileItem)
        add(file, L("新建备忘录"), #selector(newNote), "n")
        add(file, L("插入图片…"), #selector(insertImage), "")
        add(file, L("插入表格…"), #selector(insertTable), "")
        file.addItem(.separator()); add(file, L("导出富文本…"), #selector(exportNote), "")
        add(file, L("显示本地数据"), #selector(showData), "")
        file.addItem(.separator()); add(file, L("关闭窗口"), #selector(closeWindow), "w")
        let edit = NSMenu(title: L("编辑")); let editItem = NSMenuItem(title: L("编辑"), action: nil, keyEquivalent: ""); editItem.submenu = edit; main.addItem(editItem)
        for (title, selector, key) in [(L("撤销"), Selector(("undo:")), "z"), (L("重做"), Selector(("redo:")), "Z"), (L("剪切"), #selector(NSText.cut(_:)), "x"), (L("复制"), #selector(NSText.copy(_:)), "c"), (L("粘贴"), #selector(NSText.paste(_:)), "v"), (L("全选"), #selector(NSText.selectAll(_:)), "a")] { let i = NSMenuItem(title: title, action: selector, keyEquivalent: key); edit.addItem(i) }
        let format = NSMenu(title: L("格式")); let formatItem = NSMenuItem(title: L("格式"), action: nil, keyEquivalent: ""); formatItem.submenu = format; main.addItem(formatItem)
        add(format, L("粗体"), #selector(bold), "b"); add(format, L("斜体"), #selector(italic), "i"); add(format, L("下划线"), #selector(underline), "u"); add(format, L("切换待办状态"), #selector(toggleChecklist), "")
        for i in 0...4 { let item = NSMenuItem(title: i == 0 ? L("正文") : L("标题") + " \(i)", action: #selector(headingMenu(_:)), keyEquivalent: "\(i)"); item.keyEquivalentModifierMask = [.command, .option]; item.tag = i; item.target = self; format.addItem(item) }
        let display = NSMenu(title: L("显示")); let displayItem = NSMenuItem(title: L("显示"), action: nil, keyEquivalent: ""); displayItem.submenu = display; main.addItem(displayItem)
        add(display, L("显示／隐藏修改时间"), #selector(toggleTimes), "")
        add(display, L("显示／隐藏列表"), #selector(toggleSidebar), "")
    }
    ...
    func button(_ symbol: String, _ help: String, _ action: Selector) -> NSButton {
        ...
        localizedButtons.append((b, Localization.key(for: help)))
    ...
    func buildWindow() {
        ...
        window.title = L("随记"); window.titleVisibility = .hidden; window.titlebarAppearsTransparent = true
        ...
        let new = button("square.and.pencil", L("新建备忘录  ⌘N"), #selector(newNote))
        ...
        search.placeholderString = L("搜索"); search.delegate = self; search.font = .systemFont(ofSize: 12); search.controlSize = .small; search.translatesAutoresizingMaskIntoConstraints = false; search.setAccessibilityLabel(L("搜索备忘录"))
        ...
        blockPopup.addItems(withTitles: [L("正文"), L("标题 1"), L("标题 2"), L("标题 3"), L("标题 4")]); blockPopup.target = self; blockPopup.action = #selector(blockChanged)
        ...
        let formatView = NSView(frame: NSRect(x: 0, y: 0, width: 280, height: 164))
        ...
        func formatRow(_ title: String, _ control: NSView) -> NSStackView {
            ...
            localizedLabels.append((label, Localization.key(for: title)))
            label.widthAnchor.constraint(equalToConstant: 70).isActive = true
            let row = NSStackView(views: [label, control, NSView()]); row.spacing = 12
            row.heightAnchor.constraint(equalToConstant: 28).isActive = true
            return row
        }
        let styles = NSStackView(views: [button("bold", L("粗体  ⌘B"), #selector(bold)), button("italic", L("斜体  ⌘I"), #selector(italic))]); styles.spacing = 6
        let form = NSStackView(views: [formatRow(L("段落"), blockPopup), formatRow(L("字体"), fontPopup), formatRow(L("字号"), sizePopup), formatRow(L("样式"), styles)])
        ...
        let toolbar = NSStackView(views: [button("sidebar.left", L("显示或隐藏列表"), #selector(toggleSidebar)), button("textformat", L("文字格式"), #selector(showFormat(_:))), button("photo", L("插入图片"), #selector(insertImage)), button("tablecells", L("插入表格"), #selector(insertTable)), NSView(), status, button("trash", L("删除当前备忘录"), #selector(deleteNote))])
        ...
        editor.requestFormula = { [weak self] block, range in self?.editFormula(block: block, range: range) }
    ...
    func buildQuickEntry() {
        ...
        statusItem.button?.image = NSImage(systemSymbolName: "square.and.pencil", accessibilityDescription: L("快速新建备忘录"))
    ...
    @objc func quickEntry() {
        if NSApp.currentEvent?.type == .rightMouseUp {
            let menu = NSMenu(); add(menu, L("新建备忘录  ⌃⌥N"), #selector(newNote), ""); add(menu, L("打开随记"), #selector(showWindow), ""); menu.addItem(.separator()); add(menu, L("退出"), #selector(quit), "")
            statusItem.menu = menu; statusItem.button?.performClick(nil); statusItem.menu = nil
        } else { newNote() }
    }
    @objc func showWindow() { window.makeKeyAndOrderFront(nil); NSApp.activate(ignoringOtherApps: true); window.makeFirstResponder(editor.formulaDraft?.input ?? editor.view) }
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool { showWindow(); return true }
    func applicationShouldTerminateAfterLastWindowClosed(_ sender: NSApplication) -> Bool { false }
    func windowWillReturnFieldEditor(_ sender: NSWindow, to client: Any?) -> Any? {
        if let draft = editor.formulaDraft, let input = client as? NSTextField, input === draft.input { return draft.sourceEditor }
        return nil
    }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        cell.title.stringValue = note.searchable.isEmpty ? L("新备忘录") : note.title; cell.preview.stringValue = note.preview.isEmpty ? L("空白备忘录") : note.preview
        return cell
    }
    ...
    func edited() {
        ...
        let task = DispatchWorkItem { [weak self] in guard self?.editor.formulaDraft == nil else { return }; _ = self?.flush() }; saving = task
        DispatchQueue.main.asyncAfter(deadline: .now()+0.3, execute: task)
    }
    @discardableResult func flush() -> Bool {
        guard MainActor.assumeIsolated({ editor.finishFormulaEditing(cancel: false) }) else { return false }
        ...
        } catch { status.stringValue = L("保存失败，内容仍保留在窗口中"); showError(error); return false }
    }
    ...
    @objc func italic() { guard flush() else { return }; editor.formatFont(trait: .italicFontMask) }
    @objc func toggleTimes() { editor.view.showTimes.toggle() }
    @objc func toggleSidebar() { sidebar.isHidden.toggle(); split.adjustSubviews() }
    @objc func imageResized() { guard flush() else { return }; editor.resizeImage(width: imageSlider.doubleValue) }
    func editFormula(block: Bool, range: NSRange) {
        saving?.cancel(); saving = nil
        MainActor.assumeIsolated { editor.beginFormulaEditing(block: block, range: range) }
    }
    @objc func insertImage() { guard flush() else { return };
    ...
    func makeTableInsertionAlert() -> (NSAlert, NSPopUpButton, NSPopUpButton) {
        let alert = NSAlert()
        alert.messageText = L("插入表格")
        alert.informativeText = L("第一行为表头，单元格可直接编辑。")
        alert.addButton(withTitle: L("插入")); alert.addButton(withTitle: L("取消"))
        ...
        rows.addItems(withTitles: (2...20).map { "\($0) " + L("行") })
        cols.addItems(withTitles: (2...8).map { "\($0) " + L("列") })
        rows.selectItem(at: 1); cols.selectItem(at: 1)
        rows.setAccessibilityLabel(L("行数，包含表头")); cols.setAccessibilityLabel(L("列数"))
        for (title, y) in [(L("行数"), CGFloat(48)), (L("列数"), CGFloat(8))] {
    ...
    @objc func insertTable() { guard flush() else { return };
    ...
    @objc func deleteNote() {
        ...
        let alert = NSAlert(); alert.messageText = L("删除这条备忘录？"); alert.informativeText = ""; alert.addButton(withTitle: L("删除")); alert.addButton(withTitle: L("取消"))
        alert.beginSheetModal(for: window) { [weak self] response in
    ...
    @objc func exportNote() {
        ...
        let panel = NSSavePanel(); panel.nameFieldStringValue = (store.notes.first { $0.id == id }?.title ?? L("备忘录"))+".rtfd"
    ...
    @objc func about() { let alert = NSAlert(); alert.messageText = L("随记"); alert.informativeText = L("随手记下，即开即写。\n\n⌘N 新建 · ⌃⌥N 随时快速新建\n# 到 #### 加空格：标题\n**粗体**、*斜体*、`代码`：即时排版\n图片可粘贴、拖入或从工具栏插入；点击图片可调整宽度。"); alert.runModal() }
}
